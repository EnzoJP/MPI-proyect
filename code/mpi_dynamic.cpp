...
static const int DYNAMIC_CHUNK_ROWS = 4; // Number of rows to send in each task
static const int TAG_TASK = 1;
static const int TAG_RESULT = 2;
static const int TAG_DATA = 3; // tag for sending matrix data blocks
...
int main(int argc, char* argv[])
{
    ...
    // Calculate local row range for this process (same static partitioning as mpi_static)
    int baseRows = M / size;
    int extraRows = M % size;
    int localRows = baseRows + (rank < extraRows ? 1 : 0);
    int localStartRow = rank * baseRows + std::min(rank, extraRows);

    // Each process pre-generates its static chunk into `matrix` (if any).
    ...
    // Gather all chunks to root into `fullMatrix`
    int totalElems = M * R;
    ...
    // Free local per-process chunk now that root has the full matrix
    ...
    // Synchronize before starting dynamic scheduling
    ...
    if (size == 1) {
        ...
        for (int worker = 1; worker < size; ++worker) {
            if (nextRow < M) {
                ...
                // send data block from fullMatrix
        ...
        // Worker: receive tasks, use pre-generated local chunk if possible,
        // otherwise generate the requested rows temporarily and process.
        while (true) {
            int buf[2] = {0,0};
            ...
            // Receive data block from master (rowCount * R ints)
    ...
    // Reduce timing statistics across processes (generation and computation)
    double maxGenMs = 0.0, minGenMs = 0.0, sumGenMs = 0.0;
    double maxTimeMs = 0.0, minTimeMs = 0.0, sumTimeMs = 0.0;
    ...
    if (rank == 0) {
        ...
        std::cout << "-------------------------------------------------\n"
                  ...
                  << " Wall processing time        : " << totalProcMs << " ms\n"
                  << " Generation time (max/min/avg): " << maxGenMs << " / " << minGenMs << " / " << avgGenMs << " ms\n"
                  << " Computation time (max/min/avg): " << maxTimeMs << " / " << minTimeMs << " / " << avgTimeMs << " ms\n"
                  << "-------------------------------------------------\n";
    }

    // Cleanup gather arrays and fullMatrix on root
    if (rank == 0) {
        delete[] recvcounts;
        delete[] displs;
        delete[] allRows;
